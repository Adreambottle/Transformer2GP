...
def connect(sample: dict) -> str:
    ...
    output = re.sub("(" + "|".join(deliminter_list) + ")$", "", text)
...
class sampling():
    ...
    def sample_value(self, sample_features: list):
        ...
        for feature in sample_features:
            ...
            # if the feature is "MfrPartNumber", its value needs to add some random perturbation
            elif feature == "MfrPartNumber":
                x_number = 0  # The number of "x" is no more than 2
                MPN_value = random.sample(self.features_values[feature], 1)[0]
                for i in MPN_value:
                    if random.uniform(0, 1) > 0.8 and x_number < 1:  # 20% chance to replace a character with "x"
                        MPN_value = MPN_value.replace(i , 'x')
                        x_number = x_number + 1
                # Add some random deliminter
                deliminter_list = ['#', ',', '/', ';', ':', '-', '_']
                temp_list = list(MPN_value)
                temp_list.insert(random.randint(0,len(MPN_value)), random.sample(deliminter_list, 1)[0])
                MPN_value = ''.join(temp_list)
                self.sample[feature] = MPN_value
