...
def insert_deliminter(item):
    ...
    这部分是在description中添加分隔符用的
    输入是一个item，然后输出也是一个item
    会将 item 中的 description 从一个list转换成一个 str
    """
...
def get_input_from_file_single(path, inputClass, data_num, text_list, label_list, file_num, train_flag):
    """
    这个函数是用来从 Cap 和 Res 里面读取原始数据，读取的是单个的数据
...
def get_input_from_file(path, inputClass, data_num, text_list, label_list, file_num, train_flag):
    """
    这个函数是用来从 Cap 和 Res 的路径中对原始数据分块进行抽样，将切分好的数据用list储存在内存中
    ...
    return:  text_list   也是输入，然后append了新的数据
             label_list  也是输入，然后append了新的数据
    """
            ...
            if 'Resistors' in file:
                ...
                for i in range(input_size):
                    ...
                    try:
                        ...
                        text_list.append(text_new)                # 然后把新的数据添加到 text_list 里面
                    except Exception as e:
                        problematic.add(file)      # 如果出现了错误，就添加到problematic这个set里面
                        continue
...
def train(num_epoch=10, logging_steps=10, file_num=10, train_size=150, test_size=30):
    ...
    model = AutoModelForSequenceClassification.from_pretrained(model_checkpoint, num_labels=3)  # 采用的模型，这里不是很清楚？？？
    training_args = TrainingArguments(  # 模型的参数 ？？？
        output_dir='./models',
    ...
    for j in range(num_epoch):
        ...
        # i 是第几个数据，因为数据太大了，不能完全读取，现在需要修改这部分
        # file_num 是已经切分好的数据
        for i in range(file_num):
            ...
            train_dataset = processDataset(train_data, train_labels_encoded)  # processDataset，将encodings和labels合在一起
