...
def get_input_from_file_single(path, data_num, text_list, label_list, class_list, file_num, train_flag):
    ...
    # 这个是要执行多少倍的数据，在测试集上等于1，在真实数据上需要338
    execute_num = 1
    # execute_num = 338
...
class nerClass(torch.nn.Module):
    ...
    def __init__(self, config):
        ...
        self.l1 = BertModel.from_pretrained(config['model'])            # l1 是一个Bert里面的pretrain
    ...
    def forward(self, input_ids, attention_mask=None, targets=None, classes=None, device=None):
        ...
        mask1 = torch.eq(classes, 1)     # 判断 classes tensor 里面有哪个element 等于 1
        mask2 = torch.eq(classes, 2)     # 判断 classes tensor 里面有哪个element 等于 2
    ...
    for j in range(num_epoch):
        ...
        for i in range(file_num):
            ...
            train_data, train_labels_encoded, train_class = train_input_process(train_size=train_size,
                                                                                tokenizer=tokenizer, file_num=i)
...
if __name__ == "__main__":
    ...
    cap_attrs = {'Capacitance': 1,
                 'SizeCode': 2,
                 'RatedDCVoltageURdc': 3,
                 'PositiveTolerance': 4,
                 'NegativeTolerance': 5,
                 'TemperatureCharacteristicsCode': 6,
                 'MfrPartNumber': 7,
                 'input class': 8}
    res_attrs = {'Resistance': 11,
                 'SizeCode': 12,
                 'WorkingVoltage': 13,
                 'Tolerance': 14,
                 'RatedPowerDissipationP': 15,
                 'MfrPartNumber': 16,
                 'input class': 17}
