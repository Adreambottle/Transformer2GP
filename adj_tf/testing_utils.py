...
class CaptureLogger:
    ...
    Example::
        ...
        >>> from adj_tf import logging
        >>> from adj_tf.testing_utils import CaptureLogger

        >>> msg = "Testing 1, 2, 3"
        >>> logging.set_verbosity_info()
        >>> logger = logging.get_logger("adj_tf.models.bart.tokenization_bart")
...
class TestCasePlus(unittest.TestCase):
    ...
    * ``pathlib`` objects (all fully resolved):
       ...
       - ``src_dir`` - the directory of ``src`` (i.e. where the ``adj_tf`` sub-dir resides)
