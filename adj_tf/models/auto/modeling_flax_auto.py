...
class FlaxAutoModel(object):
    r"""
    :class:`~adj_tf.FlaxAutoModel` is a generic model class that will be instantiated as one of the base model
    ...
    def from_config(cls, config):
        ...
        Args:
            config (:class:`~transformers.PretrainedConfig`):
                ...
                - isInstance of `roberta` configuration class: :class:`~adj_tf.FlaxRobertaModel` (RoBERTa model)
                - isInstance of `bert` configuration class: :class:`~adj_tf.FlaxBertModel` (Bert model
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        `pretrained_model_name_or_path` string (in the following order):
            ...
            - contains `roberta`: :class:`~adj_tf.FlaxRobertaModel` (RoBERTa model)
            - contains `bert`: :class:`~adj_tf.FlaxBertModel` (Bert model)
        ...
        Args:
            pretrained_model_name_or_path: either:
                ...
                - a path to a `directory` containing model weights saved using
                  :func:`~adj_tf.FlaxPreTrainedModel.save_pretrained`, e.g.: ``./my_model_directory/``.
            ...
            config: (`optional`) instance of a class derived from :class:`~adj_tf.PretrainedConfig`:
                Configuration for the model to use instead of an automatically loaded configuration. Configuration can
                be automatically loaded when:

                - the model is a model provided by the library (loaded with the ``shortcut-name`` string of a
                  pretrained model), or
                - the model was saved using :func:`~adj_tf.FlaxPreTrainedModel.save_pretrained` and is reloaded
