...
def _list_model_options(indent, config_to_class=None, use_model_types=True):
    ...
    if use_model_types:
        ...
        lines = [
            f"{indent}- **{model_type}** -- :class:`~adj_tf.{cls_name}` ({MODEL_NAMES_MAPPING[model_type]} model)"
        ...
        lines = [
            f"{indent}- :class:`~adj_tf.{config_name}` configuration class: :class:`~adj_tf.{cls_name}` ({config_to_model_name[config_name]} model)"
...
class AutoConfig:
    ...
    when created with the :meth:`~adj_tf.AutoConfig.from_pretrained` class method.
