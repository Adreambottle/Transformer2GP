...
CTRL_START_DOCSTRING = r"""
    ...
    Parameters:
        config (:class:`~adj_tf.CTRLConfig`): Model configuration class with all the parameters of the model.
...
CTRL_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (:obj:`Numpy array` or :obj:`tf.Tensor` of shape :obj:`(batch_size, input_ids_length)`):
            ...
            Indices can be obtained using :class:`~adj_tf.CTRLTokenizer`. See
            :meth:`adj_tf.PreTrainedTokenizer.__call__` and :meth:`adj_tf.PreTrainedTokenizer.encode` for
            details.
...
class TFCTRLModel(TFCTRLPreTrainedModel):
    ...
    # Copied from adj_tf.models.gpt2.modeling_tf_gpt2.TFGPT2Model.serving_output
...
class TFCTRLLMHeadModel(TFCTRLPreTrainedModel, TFCausalLanguageModelingLoss):
    ...
    # Copied from adj_tf.models.gpt2.modeling_tf_gpt2.TFGPT2LMHeadModel.serving_output
    def serving_output(self, output):
        pkv = tf.convert_to_tensor(output.past_key_values) if self.config.use_cache else None
        hs = tf.convert_to_tensor(output.hidden_states) if self.config.output_hidden_states else None
        attns = tf.convert_to_tensor(output.attentions) if self.config.output_attentions else None
...
@add_start_docstrings(
    ...
    :class:`~adj_tf.TFCTRLForSequenceClassification` uses the last token in order to do the classification, as
