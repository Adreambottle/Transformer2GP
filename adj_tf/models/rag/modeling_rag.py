...
RAG_START_DOCSTRING = r"""
    ...
    The question encoder can be any `autoencoding` model, preferably :class:`~adj_tf.DPRQuestionEncoder`, and the
    generator can be any `seq2seq` model, preferably :class:`~adj_tf.BartForConditionalGeneration`.

    The model can be initialized with a :class:`~adj_tf.RagRetriever` for end-to-end generation or used in
    ...
    the ``generator``. It has been tested with :class:`~adj_tf.DPRQuestionEncoder` as the ``question_encoder``
    and :class:`~adj_tf.BartForConditionalGeneration` or :class:`~adj_tf.T5ForConditionalGeneration` as the
    ``generator``.
    ...
    Args:
        config (:class:`~adj_tf.RagConfig`):
            Model configuration class with all the parameters of the model. Initializing with a config file does not
            load the weights associated with the model, only the configuration. Check out the
            :meth:`~adj_tf.PreTrainedModel.from_pretrained` method to load the model weights.
        question_encoder (:class:`adj_tf.PreTrainedModel`):
            An encoder model compatible with the faiss index encapsulated by the ``retriever``.
        generator (:class:`adj_tf.PreTrainedModel`):
            A seq2seq model used as the generator in the RAG architecture.
        retriever (:class:`~adj_tf.RagRetriever`):
...
RAG_FORWARD_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (:obj:`torch.LongTensor` of shape :obj:`(batch_size, sequence_length)`):
            Indices of input sequence tokens in the vocabulary. :class:`~adj_tf.RagConfig`, used to initialize
        ...
        encoder_outputs (:obj:`tuple(tuple(torch.FloatTensor)`, `optional`)
            ...
            Used by the (:class:`~adj_tf.RagModel`) model during decoding.
        ...
        past_key_values (:obj:`tuple(tuple(torch.FloatTensor))`):
            ...
            :obj:`past_key_values` are used in the (:class:`~adj_tf.RagTokenForGeneration`) model during
            decoding.
        doc_scores (:obj:`torch.FloatTensor` of shape :obj:`(batch_size, config.n_docs)`):
            Score between each retrieved document embeddings (see :obj:`retrieved_doc_embeds`) and
        ...
        context_input_ids (:obj:`torch.LongTensor` of shape :obj:`(batch_size * config.n_docs, config.max_combined_length)`, `optional`, returned when `output_retrieved=True`):
            ...
            forward pass. :obj:`context_input_ids` are returned by :meth:`~adj_tf.RagRetriever.__call__`.
        context_attention_mask (:obj:`torch.LongTensor` of shape :obj:`(batch_size * config.n_docs, config.max_combined_length)`, `optional`, returned when `output_retrieved=True`):
            Attention mask post-processed from the retrieved documents and the question encoder :obj:`input_ids` by the
            retriever.
            ...
            :meth:`~adj_tf.RagRetriever.__call__`.
...
class RagModel(RagPreTrainedModel):
    ...
    def forward(
        ...
        Example::
            ...
            >>> from adj_tf import RagTokenizer, RagRetriever, RagModel
            >>> import torch
...
class RagSequenceForGeneration(RagPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs  # needs kwargs for generation
        ...
        Example::
            ...
            >>> from adj_tf import RagTokenizer, RagRetriever, RagSequenceForGeneration
            >>> import torch
    ...
    def generate(
        ...
        **model_kwargs
        ...
        """
        Implements RAG sequence "thorough" decoding. Read the :meth:`~adj_tf.PreTrainedModel.generate``
        ...
        Args:
            ...
            context_attention_mask (:obj:`torch.LongTensor` of shape :obj:`(batch_size * config.n_docs, config.max_combined_length)`, `optional`, returned when `output_retrieved=True`):
                ...
                They are returned by :meth:`~adj_tf.RagRetriever.__call__`.
            doc_scores (:obj:`torch.FloatTensor` of shape :obj:`(batch_size, config.n_docs)`):
                Score between each retrieved document embeddings (see :obj:`retrieved_doc_embeds`) and
                :obj:`question_encoder_last_hidden_state`.
            ...
            num_return_sequences(:obj:`int`, `optional`, defaults to 1):
                ...
                is not the value we pass to the ``generator``'s `:func:`~adj_tf.PreTrainedModel.generate``
            ...
            n_docs (:obj:`int`, `optional`, defaults to :obj:`config.n_docs`)
                Number of documents to retrieve and/or number of documents for which to generate an answer.
            kwargs:
                Additional kwargs will be passed to :meth:`~adj_tf.PreTrainedModel.generate`.
    ...
    def forward(
        ...
        **kwargs  # needs kwargs for generation
        ...
        Example::
            ...
            >>> from adj_tf import RagTokenizer, RagRetriever, RagTokenForGeneration
            >>> import torch
