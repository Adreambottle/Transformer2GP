...
path = /path/to/adj_tf/.env/bin/python

args = -m debugpy --listen 5678 --wait-for-client /path/to/adj_tf/src/adj_tf/commands/transformers_cli.py
...
class LfsCommands(BaseTransformersCLICommand):
    ...
    1. $ adj_tf-cli lfs-enable-largefiles
    ...
    2. $ adj_tf-cli lfs-multipart-upload
...
class LfsEnableCommand:
    ...
    def run(self):
        ...
        if not os.path.isdir(local_path):
            ...
            exit(1)
        subprocess.run(
            "git config lfs.customtransfer.multipart.path adj_tf-cli".split(), check=True, cwd=local_path
        )
        subprocess.run(
