    ...
    def run(self):
        if not _has_cookiecutter:
            raise ImportError(
                ...
                "the folowing at the root of your `adj_tf` clone:\n\n\t$ pip install -e .[modelcreation]\n"
            )
        ...
        model_dir = f"{path_to_transformer_root}/src/adj_tf/models/{lowercase_model_name}"
        ...
        def remove_copy_lines(path):
            ...
            with open(path, "w") as f:
                for line in lines:
                    if "# Copied from adj_tf." not in line:
                        f.write(line)
