...
class TensorBoardCallback(TrainerCallback):
    """
    A :class:`~adj_tf.TrainerCallback` that sends the logs to `TensorBoard
...
class WandbCallback(TrainerCallback):
    """
    A :class:`~adj_tf.TrainerCallback` that sends the logs to `Weight and Biases <https://www.wandb.com/>`__.
    """
...
class CometCallback(TrainerCallback):
    """
    A :class:`~adj_tf.TrainerCallback` that sends the logs to `Comet ML <https://www.comet.ml/site/>`__.
    """
    ...
    def setup(self, args, state, model):
        ...
        if state.is_world_process_zero:
            ...
            if experiment is not None:
                experiment._set_model_graph(model, framework="adj_tf")
                experiment._log_parameters(args, prefix="args/", framework="adj_tf")
                if hasattr(model, "config"):
                    experiment._log_parameters(model.config, prefix="config/", framework="adj_tf")
    ...
    def on_log(self, args, state, control, model=None, logs=None, **kwargs):
        ...
        if state.is_world_process_zero:
            ...
            if experiment is not None:
                experiment._log_metrics(logs, step=state.global_step, epoch=state.epoch, framework="adj_tf")


class AzureMLCallback(TrainerCallback):
    """
    A :class:`~adj_tf.TrainerCallback` that sends the logs to `AzureML
...
class MLflowCallback(TrainerCallback):
    """
    A :class:`~adj_tf.TrainerCallback` that sends the logs to `MLflow <https://www.mlflow.org/>`__.
    """
