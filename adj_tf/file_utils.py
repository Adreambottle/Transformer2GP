...
old_default_cache_path = os.path.join(torch_cache_home, "adj_tf")
# New default cache, shared with the Datasets library
hf_cache_home = os.path.expanduser(
    os.getenv("HF_HOME", os.path.join(os.getenv("XDG_CACHE_HOME", "~/.cache"), "huggingface"))
)
default_cache_path = os.path.join(hf_cache_home, "adj_tf")
...
if (
    ...
    logger.warn(
        ...
        "'~/.cache/torch/adj_tf' to '~/.cache/huggingface/adj_tf'. Since you don't seem to have overridden "
        "and '~/.cache/torch/adj_tf' is a directory that exists, we're moving it to "
        "'~/.cache/huggingface/adj_tf' to avoid redownloading models you have already in the cache. You should "
...
def add_start_docstrings_to_model_forward(*docstr):
    def docstring_decorator(fn):
        class_name = ":class:`~adj_tf.{}`".format(fn.__qualname__.split(".")[0])
...
PT_RETURN_INTRODUCTION = r"""
    Returns:
        ...
        comprising various elements depending on the configuration (:class:`~adj_tf.{config_class}`) and inputs.
...
TF_RETURN_INTRODUCTION = r"""
    Returns:
        ...
        various elements depending on the configuration (:class:`~adj_tf.{config_class}`) and inputs.
...
PT_TOKEN_CLASSIFICATION_SAMPLE = r"""
    Example::
        ...
        >>> from adj_tf import {tokenizer_class}, {model_class}
        >>> import torch
...
def hf_bucket_url(
    ...
    its sha1 if stored in git, or its sha256 if stored in git-lfs. Files cached locally from adj_tf before v3.5.0
...
def get_cached_models(cache_dir: Union[str, Path] = None) -> List[Tuple]:
    ...
    Args:
        cache_dir (:obj:`Union[str, Path]`, `optional`):
            The cache directory to search for models within. Will default to the adj_tf cache if unset.
...
def http_user_agent(user_agent: Union[Dict, str, None] = None) -> str:
    ...
    """
    ua = "adj_tf/{}; python/{}".format(__version__, sys.version.split()[0])
    if is_torch_available():
...
class ModelOutput(OrderedDict):
    ...
    .. warning::
        You can't unpack a :obj:`ModelOutput` directly. Use the :meth:`~adj_tf.file_utils.ModelOutput.to_tuple`
