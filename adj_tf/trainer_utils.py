...
class BestRun(NamedTuple):
    """
    The best run found by an hyperparameter search (see :class:`~adj_tf.Trainer.hyperparameter_search`).
...
def default_compute_objective(metrics: Dict[str, float]) -> float:
    ...
    metrics are provided to the :class:`~adj_tf.Trainer`, the sum of all metrics otherwise.
...
class TrainerMemoryTracker:
    ...
    Note that this tracker doesn't account for memory allocations outside of :class:`~adj_tf.Trainer`'s
    ...
    This also means that if any other tool that is used along the :class:`~adj_tf.Trainer` calls
    ``torch.cuda.reset_peak_memory_stats``, the gpu peak memory stats could be invalid. And the
    :class:`~adj_tf.Trainer` will disrupt the normal behavior of any such tools that rely on calling
