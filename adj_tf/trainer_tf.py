...
class TFTrainer:
    ...
    Args:
        ...
        optimizers (:obj:`Tuple[tf.keras.optimizers.Optimizer, tf.keras.optimizers.schedules.LearningRateSchedule]`, `optional`):
            ...
            :class:`~adj_tf.AdamWeightDecay`. The scheduler will default to an instance of
            :class:`tf.keras.optimizers.schedules.PolynomialDecay` if :obj:`args.num_warmup_steps` is 0 else an
            instance of :class:`~adj_tf.WarmUp`.
    """
    ...
    def setup_comet(self):
        ...
        if experiment is not None:
            experiment._set_model_graph(self.model, framework="adj_tf")
            experiment._log_parameters(self.args, prefix="args/", framework="adj_tf")
            experiment._log_parameters(self.model.config, prefix="config/", framework="adj_tf")
    ...
    def prediction_loop(
        ...
        """
        Prediction/evaluation loop, shared by :func:`~adj_tf.TFTrainer.evaluate` and
        :func:`~adj_tf.TFTrainer.predict`.
        ...
        if is_comet_available():
            ...
            if experiment is not None:
                experiment._log_metrics(
                    logs, step=self.global_step, epoch=self.epoch_logging, framework="adj_tf"
                )
