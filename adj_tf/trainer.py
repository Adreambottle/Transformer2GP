...
class Trainer:
    ...
    Args:
        model (:class:`~adj_tf.PreTrainedModel` or :obj:`torch.nn.Module`, `optional`):
            The model to train, evaluate or use for predictions. If not provided, a ``model_init`` must be passed.

            .. note::

                :class:`~adj_tf.Trainer` is optimized to work with the :class:`~adj_tf.PreTrainedModel`
                provided by the library. You can still use your own models defined as :obj:`torch.nn.Module` as long as
                they work the same way as the 🤗 Transformers models.
        args (:class:`~adj_tf.TrainingArguments`, `optional`):
            The arguments to tweak for training. Will default to a basic instance of
            :class:`~adj_tf.TrainingArguments` with the ``output_dir`` set to a directory named `tmp_trainer` in
            the current directory if not provided.
        data_collator (:obj:`DataCollator`, `optional`):
            The function to use to form a batch from a list of elements of :obj:`train_dataset` or :obj:`eval_dataset`.
            Will default to :func:`~adj_tf.default_data_collator` if no ``tokenizer`` is provided, an instance of
            :func:`~adj_tf.DataCollatorWithPadding` otherwise.
        ...
        model_init (:obj:`Callable[[], PreTrainedModel]`, `optional`):
            ...
            :meth:`~adj_tf.Trainer.train` will start from a new instance of the model as given by this function.
        ...
        compute_metrics (:obj:`Callable[[EvalPrediction], Dict]`, `optional`):
            ...
            :class:`~adj_tf.EvalPrediction` and return a dictionary string to metric values.
        callbacks (List of :obj:`~adj_tf.TrainerCallback`, `optional`):
        ...
        optimizers (:obj:`Tuple[torch.optim.Optimizer, torch.optim.lr_scheduler.LambdaLR`, `optional`): A tuple
            ...
            :class:`~adj_tf.AdamW` on your model and a scheduler given by
            :func:`~adj_tf.get_linear_schedule_with_warmup` controlled by :obj:`args`.

    Important attributes:

        - **model** -- Always points to the core model. If using a adj_tf model, it will be a
          :class:`~adj_tf.PreTrainedModel` subclass.
        ...
        **kwargs,
        ...
        Args:
            resume_from_checkpoint (:obj:`str`, `optional`):
                Local path to a saved checkpoint as saved by a previous instance of :class:`~adj_tf.Trainer`. If
    ...
    def hyperparameter_search(
        ...
        **kwargs,
        ...
        .. warning::
            ...
            :class:`~adj_tf.Trainer`: we need to reinitialize the model at each new run. This is incompatible
            with the ``optimizers`` argument, so you need to subclass :class:`~adj_tf.Trainer` and override the
            method :meth:`~adj_tf.Trainer.create_optimizer_and_scheduler` for custom optimizer/scheduler.

        Args:
            hp_space (:obj:`Callable[["optuna.Trial"], Dict[str, float]]`, `optional`):
                A function that defines the hyperparameter search space. Will default to
                :func:`~adj_tf.trainer_utils.default_hp_space_optuna` or
                :func:`~adj_tf.trainer_utils.default_hp_space_ray` depending on your backend.
            compute_objective (:obj:`Callable[[Dict[str, float]], float]`, `optional`):
                A function computing the objective to minimize or maximize from the metrics returned by the
                :obj:`evaluate` method. Will default to :func:`~adj_tf.trainer_utils.default_compute_objective`.
            ...
            direction(:obj:`str`, `optional`, defaults to :obj:`"minimize"`):
                ...
                several metrics.
            backend(:obj:`str` or :class:`~adj_tf.training_utils.HPSearchBackend`, `optional`):
    ...
    def floating_point_ops(self, inputs: Dict[str, Union[torch.Tensor, Any]]):
        """
        For models that inherit from :class:`~adj_tf.PreTrainedModel`, uses that method to compute the number of
